...
function set_time(){
    let timeLeft = display_time_period_from_seconds(secondsUntilArrival);
}


function set_train_arrivals( intervalInSeconds ){
    let runTimeConfig = get_runtime_config();
    let stationCodeToNameMap = runTimeConfig.transport.stationCodeToNameMap;

    let model = {};
    model.commutes = runTimeConfig.transport.commutes;
    model.maximumTrainsToShow = runTimeConfig.transport.maximumTrainsToShow;
    model.transportApi = get_runtime_config().transportApi
    model.stationCodeToNameMap = new Map(Object.entries(stationCodeToNameMap));

   for( i = 0; i < model.commutes.length; i++ ){
        set_train_station_arrivals( model.commutes[i], model, runTimeConfig.transportApi );
    }
...
function show_or_hide_school_run_departure_time(){
    let schoolRunCountDown = get_runtime_config().schoolRunCountDown;
...
function build_arrival_string( arrivalTime,  walkTransitTime, runTransitTime, driveTransitTime ){
    ...
    var colouredEtaString = colour_eta_warnings( secondsUntilArrival , arrivalString, tooEarly, walkTransitTime, runTransitTime, driveTransitTime );
    return colouredEtaString;
}
